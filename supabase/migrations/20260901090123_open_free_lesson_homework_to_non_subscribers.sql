-- واجب الدرس المجاني يتفتح لغير المشترك. أي واجب تاني (درس مدفوع أو بنك أسئلة
-- مش مربوط بدرس) يفضل مقفول على المشترك بس زي ما هو بالظبط.

create or replace function public.quiz_is_free_lesson_homework(p_quiz_id uuid)
returns boolean
language sql
stable
security definer
...
revoke all on function public.quiz_is_free_lesson_homework(uuid) from public, anon;
grant execute on function public.quiz_is_free_lesson_homework(uuid) to authenticated;

-- 1) رؤية الواجب في القوائم
...
-- 2) قائمة واجبات الطالب (التطبيق)
create or replace function public.get_available_quizzes_for_student()
 returns table(id uuid, title text, description text, course_id uuid, course_title text, lesson_id uuid, lesson_title text, time_limit_minutes integer, total_marks integer, pass_marks integer, question_count bigint, attempt_count bigint, best_percentage integer, last_result_id uuid, last_score integer, last_total_marks integer, last_passed boolean, last_taken_at timestamp with time zone)
 language plpgsql
 security definer
 set search_path to ''
as $function$
...
begin
  ...
  where q.is_published = true
    and c.is_published = true
    and (
      public.has_active_course_access(v_user_id, q.course_id)
      or public.quiz_is_free_lesson_homework(q.id)
    )
...
create or replace function public.get_quiz_questions_for_student(p_quiz_id uuid)
 returns table(id uuid, quiz_id uuid, question_text text, option_a text, option_b text, option_c text, option_d text, marks integer, order_index integer, question_image_url text, question_link_url text, question_link_text text)
 language plpgsql
 security definer
 set search_path to ''
as $function$
